...
func avoid():
	...
	for n in neighbors:
		var distance = n.position.distance_to(position)
		if distance > 0:
			var factor = 1 - (distance - 2*$Body.shape.radius) / ($Space/SpaceShape.shape.radius - $Body.shape.radius)
			if factor > 0:
...
func _draw():
	...
	if !DEBUG_DRAW:
		return
	draw_circle(Vector2.ZERO, $Body.shape.radius,Color(1, 0, 1, 0.4))
	#draw_circle(Vector2.ZERO, $Space/SpaceShape.shape.radius,Color(1, 1, 0, 0.2))
